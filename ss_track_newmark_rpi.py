...
#        return offset
    ...
    def coarse_track(self):
        ...
        if self.ss4.sun_in_fov == True:
            ...
            self.coarse_move_time_x = int(self.coarse_steps_x/self.coarse_vel)
            ...
            time.sleep(max(self.coarse_move_time_x,self.coarse_move_time_y) + self.coarse_settle_t)
...
######################### Read Fine Sun Sensors ###############################
            ...
            for i in ss_read:    #Loop through all sun sensors in ss_read
                ...
                if i in self.ss_track:   #Only include x and y SS offsets if included in ss_track
                    ...
                    #Set coarse tracking to true only if one of sun sensors in ss_track lost the sun
                    if i == 1:
...
##################### PTU Logic ###############################################
            ...
            #Only issue PTU commands if coarse_track = False
