...
def sunsensor_read( delay,
    ...
    while True:
        ...
        ss1.read_data_raw() #Read all data from sun sensor using SS class      
        ...
        if filter_mode == 2:   #Use filtered data if filter mode=2, otherwise use raw data
            ...
            if(SS2_queue.full()==False):
                SS2_queue.put([ss1.ang_x_raw + ss_eshim_x[1],ss2.ang_y_raw + ss_eshim_y[1],ss2_tstamp])
        ...
        if filter_mode == 2:   #Use filtered data if filter mode=2, otherwise use raw data
            if(SS3_queue.full()==False):
                SS3_queue.put([ss1.ang_x_filt + ss_eshim_x[2],ss3.ang_y_filt + ss_eshim_y[2],ss3_tstamp])   #add electronic shims to angle offset for tracking  
        else:
            if(SS3_queue.full()==False):
